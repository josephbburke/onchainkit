...
REDIS_TOKEN=`
  ...
  if (setUpFrame) {
    ...
    generateAccountAssociation.on('close', (code: number) => {
      if (code === 0) {
        logMiniKitSetupSummary(projectName, root, smartWallet, clientKey);
      } else {
...
function logMiniKitSetupSummary(projectName: string, root: string, smartWallet: boolean, clientKey: string) {
  console.log(`\nIntegrations:`);
  if (smartWallet) {
